...
def test_short_dataset_v2_blocks_when_required_sources_missing_and_downloads_disabled(tmp_path: Path, monkeypatch):
...
def test_short_dataset_v2_quota_backfill_uses_candidate_backed_rules_and_reports(tmp_path: Path):
    ...
    quota = pd.read_csv(tmp_path / "reports" / "short_dataset_v2" / "active_rule_quota_report.csv")
    rejected = pd.read_csv(tmp_path / "reports" / "short_dataset_v2" / "rejected_backfill_templates.csv")

    assert result["verdict"] == "READY_FOR_SHORT_TRAINING_DATASET_V2"
...
def _tiny_quota_config(tmp_path: Path) -> dict:
    ...
    v2["min_clean_pool_for_ready"] = 40
    v2["min_clean_pool_hard_min"] = 40
    v2["pool"]["min_clean_sentences"] = 40
    v2["pool"]["max_source_share"] = 1.0
    v2["pool"]["max_subcorpus_share"] = 1.0
    v2["audit"]["min_active_rule_count"] = 3
    v2["audit"]["require_all_source_types"] = False
    v2["active_rule_quota"] = {
        "rule_ids": [
            "comma_subordinate",
            "homogeneous_comma",
            "subject_predicate_dash",
