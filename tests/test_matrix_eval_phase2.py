...
def test_under_quota_audit_classifies_backfilled_and_no_candidate_path(tmp_path: Path):
    ...
    audit = write_under_quota_rule_audit(
        ...
        phase2_summary_path=summary,
        phase2_dataset_path=dataset,
...
def test_phase2_wave1_plan_and_backlog_apply_safety_gates(tmp_path: Path):
    ...
    pd.DataFrame(
        ...
        columns=BACKLOG_V2_COLUMNS,
    ).to_csv(backlog, index=False)

    wave = write_wave1_activation_plan(
        summary_path=summary_path,
        under_quota_audit_path=under_quota_path,
        backlog_v2_path=backlog,
        output_csv_path=tmp_path / "wave1_activation_plan.csv",
        output_md_path=tmp_path / "wave1_activation_plan.md",
    )

    assert list(wave.columns) == WAVE1_ACTIVATION_COLUMNS
    by_rule = wave.set_index("rule_id")
    assert by_rule.loc["address_comma", "wave1_decision"] == "INCLUDE"
    assert by_rule.loc["pattern_чо_че", "wave1_decision"] == "EXCLUDE"
    assert "NEEDS_THRESHOLD_CALIBRATION" in by_rule.loc["pattern_чо_че", "blockers"]


def test_phase2_backlog_uses_required_blocker_taxonomy(tmp_path: Path):
    ...
    backlog = write_rule_expansion_backlog_v2(
        inventory_path=inventory_path,
        summary_path=summary_path,
        output_csv_path=tmp_path / "rule_expansion_backlog_v2.csv",
        output_md_path=tmp_path / "rule_expansion_backlog_v2.md",
    )

    assert list(backlog.columns) == BACKLOG_V2_COLUMNS
    blockers = set(backlog["blocker"])
    assert "NEEDS_NER_SUPPORT" in blockers
    assert "NEEDS_VALIDATOR" in blockers


def test_phase2_audit_rejects_positive_rows_without_candidates(tmp_path: Path):
    root = tmp_path / "reports"
    data = tmp_path / "data"
    eval_dir = root / "working_v1_eval"
    root.mkdir()
    data.mkdir()
    eval_dir.mkdir()
    pd.DataFrame(
    ...
    for name in ("under_quota_rule_audit.csv", "validator_fix_report.md", "wave1_activation_plan.csv", "rule_expansion_backlog_v2.csv"):
        path = root / name
        if path.suffix == ".csv":
    ...
    audit = assert_matrix_eval_phase2_audit(root_dir=root, data_dir=data, eval_dir=eval_dir)

    assert audit["verdict"] == "MATRIX_EVAL_PHASE2_BLOCKED"
