...
def test_main_config_is_canonical_train_profile_for_candidate_opportunity_dataset():
    ...
    assert config["training"]["max_train_examples"] == 192000
    assert config["training"]["max_val_examples"] == 24000
    assert config["training"]["max_test_examples"] == 24000
    assert set(config["data"]) == {"candidate_opportunity"}
    assert candidate["contract"] == "candidate_opportunity"
    assert candidate["dataset_build_workers"] == 8
    assert totals == {
    ...
    assert candidate["clean_pool"]["reject_mixed_script_tokens"] is True
    assert candidate["clean_pool"]["reject_latin_confusable_inside_cyrillic_word"] is True
    assert candidate["clean_pool"]["reject_if_candidate_generator_finds_high_confidence_fix"] is True
    assert candidate["clean_pool"]["high_confidence_candidate_threshold"] == 0.95
    assert candidate["synthetic"]["require_atomic_positive"] is True
    assert candidate["synthetic"]["max_gold_edits_for_atomic"] == 1
    assert candidate["synthetic"]["require_candidate_coverage"] is True
    assert candidate["synthetic"]["require_strict_validator_acceptance"] is True
    assert candidate["real_pairs"]["train_policy"] == "atomize_single_edit_known_rule_only"
    assert candidate["real_pairs"]["unknown_rule_policy"] == "mining_only"
    assert candidate["real_pairs"]["multi_edit_policy"] == "stress_or_eval_only"
    assert candidate["stress"]["enabled"] is True
    assert candidate["stress"]["count_toward_rule_quota"] is False
    assert candidate["stress"]["loss_weight"] == 0.4
    assert candidate["stress"]["min_ratio"] == 0.03
    assert candidate["stress"]["max_ratio"] == 0.05
    assert candidate["stress"]["fail_on_under_target"] is False
    assert candidate["rule_activation"]["expected_min_training_candidate_rule_count"] == 76
    assert candidate["rule_activation"]["target_training_candidate_rule_count"] == 76
    assert candidate["rule_activation"]["expected_min_final_active_rule_count"] == 25
    assert candidate["rule_activation"]["target_final_active_rule_count"] == 76
    assert candidate["rule_activation"]["fail_below_final_active_rule_count"] is True
    assert candidate["rule_activation"]["warn_below_target_final_active_rule_count"] is True
    assert candidate["rule_quota"]["min_atomic_positives_per_active_rule"] == 500
    assert candidate["rule_quota"]["preferred_atomic_positives_per_active_rule"] == 1500
    assert candidate["rule_quota"]["max_total_per_rule_id"] == 1500
    assert candidate["rule_quota"]["min_hard_negatives_per_active_rule"] == 200
    assert candidate["rule_quota"]["disable_rule_if_quota_not_met"] is True
    assert candidate["rule_data_compiler"] == {
        "enabled": True,
        "source_priority": [
            "corpus_mined",
            "syntax_mined",
    ...
    assert candidate["audit"]["fail_on_extra_edits_in_atomic"] is True
    assert candidate["audit"]["fail_on_unknown_rule_in_train"] is True
    assert candidate["audit"]["fail_on_stale_reports"] is True
    assert candidate["audit"]["min_candidate_recall_for_active_rule"] == 0.95
    assert candidate["audit"]["fail_on_corpus_opportunity_share_below_threshold"] is False
    assert candidate["audit"]["destructive_diversity_pruning_enabled"] is False
