...
PROFILE_NAME = "calibrated_val_guarded_v2"
V1_PROFILE_NAME = "calibrated_val_guarded"
...
def run_threshold_safety_v2(
    *,
    ...
    config = _config_with_profile(config, V1_PROFILE_NAME, v1_profile)
    ...
    if not score_expanded_safety:
        ...
        return {
            ...
            "recommended_thresholds_path": str(output / "recommended_thresholds_v2.yaml"),
            "val_config_path": "/tmp/config.eval_val_calibrated_v2.yaml",
            "test_config_path": "/tmp/config.eval_test_calibrated_v2.yaml",
        }
    ...
    selection_cache = _score_rows(config, corrector, selection_rows.to_dict("records"), "threshold_safety_v2", eval_batch_size)
...
def _write_selected_profile_outputs(
    *,
    ...
    (output / "recommended_thresholds_v2.yaml").write_text(
        yaml.safe_dump(recommended_payload, allow_unicode=True, sort_keys=False),
        encoding="utf-8",
    )
    pd.DataFrame(selected.sweep_rows).to_csv(output / "threshold_safety_sweep_v2.csv", index=False)
    pd.DataFrame(selected.decision_rows).to_csv(output / "threshold_safety_decision_v2.csv", index=False)
    _write_threshold_safety_summary(
        output / "threshold_safety_summary_v2.md",
        selected=selected,
    ...
    write_tmp_eval_config(
        ...
        profile=selected.profile,
        split="val",
        reports_dir="reports/eval_val_calibrated_v2_short_v2",
        output_path="/tmp/config.eval_val_calibrated_v2.yaml",
    )
    write_tmp_eval_config(
        config=config,
        profile=selected.profile,
        split="test",
        reports_dir="reports/eval_test_calibrated_v2_short_v2",
        output_path="/tmp/config.eval_test_calibrated_v2.yaml",
    )
...
def maybe_update_config_with_v2(
    *,
    config_path: str | Path,
    profile: dict[str, float],
    val_metrics: dict[str, float],
    ...
    backup_path = path.with_name(f"{path.name}.backup_threshold_v2_{time.strftime('%Y%m%d_%H%M%S')}")
...
def write_risky_threshold_impact_report(
    *,
    ...
    calibration_dir = Path("reports/short_dataset_v2/threshold_calibration/20260520_173850")
    ...
    score_distribution = _read_csv(Path("reports/eval_test_calibrated_short_v2/candidate_score_distribution_by_rule.csv"))
    rows = []
    ...
    output = Path(output_dir)
    output.mkdir(parents=True, exist_ok=True)
    frame.to_csv(output / "risky_threshold_impact_v2.csv", index=False)
    return frame
...
def _select_profile_from_existing_reports(v1_profile: dict[str, float]) -> ProfileSelectionResult:
    ...
    val_metrics = _load_metrics("reports/eval_val_calibrated_short_v2/evaluation_summary.csv")
...
def _existing_report_selection_reason(key: str) -> str:
    return {
        ...
        "double_consonant_candidate_threshold": "raise per v2 candidate grid: val calibration admitted clean false positives with low positive score support",
        "swapped_letters_candidate_threshold": "raise per v2 candidate grid: synthetic-only gain and expanded clean safety priority",
        "hyphen_po_adverbs_threshold": "raise per v2 candidate grid: synthetic-only gain with high negative-score risk",
...
def _load_v1_profile(path: str | Path) -> dict[str, float]:
    ...
    profile = payload.get(V1_PROFILE_NAME) or payload.get("calibrated_val_guarded")
    if not isinstance(profile, dict):
        raise ValueError(f"missing {V1_PROFILE_NAME} profile in {path}")
...
def _default_recommendation(rule_id: str, threshold_key: str) -> str:
    if threshold_key in ROLLBACK_SWEEPS:
        return "raise threshold candidate during v2 selection"
...
def _risk_recommendation(rule_id: str, test_rows: pd.DataFrame, real_support: int) -> str:
    ...
    if clean_fp or real_dirty:
        return "raise or rollback in v2"
    if real_support < 2:
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Build calibrated_val_guarded_v2 threshold safety reports.")
    parser.add_argument("--config", default="configs/config.yaml")
    parser.add_argument("--dataset", default="data/processed/short_dataset_v2/correction_dataset.csv.gz")
    parser.add_argument("--v1-thresholds", default="reports/short_dataset_v2/threshold_calibration/latest_recommended_thresholds.yaml")
    parser.add_argument("--test-report-dir", default="reports/eval_test_calibrated_short_v2")
    parser.add_argument("--output-dir", default="reports/threshold_safety_v2")
    ...
    result = run_threshold_safety_v2(
