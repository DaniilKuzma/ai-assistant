...
| `ne_ni_guards.yaml` | `semantic_ne_ni` | none | Guard-only. No word insertion such as adding `как`. | `ne_kto_inoy_guard`, `nikto_inoy_guard`, `ne_chto_inoe_guard`, `nichto_inoe_guard` |
| `introductory_context.yaml` | `semantic_introductory_context` | none | Guard-only to avoid duplicating `punct_introductory_extended` positives. | `introductory_homonym_guard` |
...
| Rule id | Positive | Hard negative |
| --- | ---: | ---: |
| `semantic_service_words` | 261 | 32 |
| `semantic_derived_prepositions` | 141 | 20 |
| `semantic_ne_ni` | 0 | 16 |
| `semantic_introductory_context` | 0 | 5 |
| `semantic_comparative_context` | 52 | 4 |
