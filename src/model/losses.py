...
def multitask_loss_with_components(outputs: dict, labels: dict, weights: dict[str, float]):
    ...
    if "candidate_labels" in labels:
        ...
        word_loss = _apply_sample_weight(word_loss, sample_weight)
    ...
    if "punctuation_labels" in labels:
        ...
        punctuation_loss = _apply_sample_weight(punctuation_loss, sample_weight)
    ...
    if "punctuation_action_labels" in labels and "punctuation_action_logits" in outputs:
        ...
        punctuation_action_loss = _apply_sample_weight(punctuation_action_loss, sample_weight)
    ...
    if "confidence_labels" in labels:
        ...
        confidence_loss = _apply_sample_weight(confidence_loss, sample_weight)
    ...
    if "punctuation_confidence_labels" in labels and "punctuation_confidence_logits" in outputs:
        ...
        punctuation_confidence_loss = _apply_sample_weight(punctuation_confidence_loss, sample_weight)
...
def _apply_sample_weight(values, sample_weight):  # type: ignore[no-untyped-def]
    if sample_weight is None:
        return values
    return values * _expand_sample_weight(sample_weight, values)


def _expand_sample_weight(sample_weight, target, *, dtype=None):  # type: ignore[no-untyped-def]
    weight = sample_weight.to(device=target.device, dtype=dtype or target.dtype)
    while weight.ndim < target.ndim:
        weight = weight.unsqueeze(-1)
    return weight.expand(target.shape)
...
def _cross_entropy_ignore_empty(logits, labels, sample_weight=None):  # type: ignore[no-untyped-def]
    import torch.nn.functional as functional
    ...
    losses = functional.cross_entropy(flat_logits[valid_mask], flat_labels[valid_mask], reduction="none")
    if sample_weight is not None:
        flat_sample_weight = _expand_sample_weight(sample_weight, labels, dtype=logits.dtype).reshape(-1)
        losses = losses * flat_sample_weight[valid_mask]
    return losses.sum() / valid_mask.sum().to(logits.dtype).clamp_min(1.0)
