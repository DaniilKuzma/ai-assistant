...
PHASE2_REQUIRED_REPORTS = [
    "under_quota_rule_audit.csv",
    "validator_fix_report.md",
    "wave1_activation_plan.csv",
    "rule_expansion_backlog_v2.csv",
]

PHASE2_REQUIRED_EVAL_REPORTS = [
...
def write_under_quota_rule_audit(
    *,
    ...
    phase2_dataset_path: str | Path,
    output_path: str | Path,
) -> pd.DataFrame:
    manifest = _read_json(phase1_manifest_path)
    inventory = _read_csv(inventory_path)
    summary = _read_csv(phase2_summary_path)
    dataset = _read_csv(phase2_dataset_path)
    target = int(manifest.get("quota", {}).get("min_eval_examples_per_executable_rule", 50))
    phase1_counts = {str(key): int(value) for key, value in (manifest.get("rule_id_counts") or {}).items()}
    ...
    phase2_counts = _dataset_rule_counts(dataset)
    ...
    for rule_id in sorted(rule_id for rule_id in underfilled if rule_id != UNKNOWN_RULE_ID):
        ...
        current_count = int(phase1_counts.get(rule_id, _number(inv.get("current_eval_count"), 0)))
        phase2_count = int(phase2_counts.get(rule_id, _number(item.get("eval_examples"), 0)))
        ...
        backfill_success = max(0, phase2_count - current_count)
...
def write_rule_expansion_backlog_v2(
    *,
    inventory_path: str | Path,
    summary_path: str | Path,
    ...
    frame = pd.DataFrame(rows, columns=BACKLOG_V2_COLUMNS).sort_values(["priority", "rule_id"], kind="stable")
    _write_csv(frame, output_csv_path)
    _write_backlog_md(frame, output_md_path)
    return frame


def write_wave1_activation_plan(
    *,
    summary_path: str | Path,
    under_quota_audit_path: str | Path,
    backlog_v2_path: str | Path,
    output_csv_path: str | Path,
    output_md_path: str | Path,
) -> pd.DataFrame:
    summary = _read_csv(summary_path)
    under_quota = _read_csv(under_quota_audit_path)
    backlog = _read_csv(backlog_v2_path)
    ...
    for item in summary.to_dict("records"):
        ...
        include, reason = _wave1_decision(item, under_decision, blockers)
        rows.append(
            {
                "rule_id": rule_id,
                ...
                "wave1_decision": "INCLUDE" if include else "EXCLUDE",
                "reason": reason,
            }
        )
    frame = pd.DataFrame(rows, columns=WAVE1_ACTIVATION_COLUMNS).sort_values(["wave1_decision", "rule_id"], kind="stable")
    _write_csv(frame, output_csv_path)
    _write_wave1_md(frame, output_md_path)
    return frame


def assert_matrix_eval_phase2_audit(
    *,
    root_dir: str | Path = "reports/matrix_eval_phase2",
    data_dir: str | Path = "data/processed/matrix_eval_phase2",
    ...
    missing: list[str] = []
    errors: list[str] = []
    for name in PHASE2_REQUIRED_REPORTS:
        if not (root / name).exists():
            missing.append(name)
    for name in ("matrix_eval.csv.gz", "matrix_eval_manifest.json"):
        if not (data / name).exists():
            missing.append(name)
    for name in PHASE2_REQUIRED_EVAL_REPORTS:
    ...
    if Path(config_path).exists():
        ...
        if "mode: calibrated_val_guarded_v3" not in text:
            errors.append("working v1 threshold profile is not calibrated_val_guarded_v3")
    ...
    if errors:
        verdict = "MATRIX_EVAL_PHASE2_BLOCKED"
    elif missing:
        verdict = "MATRIX_EVAL_PHASE2_PARTIAL"
    else:
        verdict = "MATRIX_EVAL_PHASE2_COMPLETE"
    result = {"verdict": verdict, "missing_outputs": missing, "errors": errors}
    summary_path = root / "phase2_audit_summary.json"
...
def write_phase2_final_report(
    *,
    ...
    backlog = _read_csv(root / "rule_expansion_backlog_v2.csv")
    wave = _read_csv(root / "wave1_activation_plan.csv")
    manifest = _read_json(Path(data_dir) / "matrix_eval_manifest.json")
    lines = [
        "# Matrix Eval Phase 2 Final Report",
        "",
        ...
        f"- phase2 evaluated rule count: {summary['rule_id'].nunique() if 'rule_id' in summary.columns else 0}",
        f"- phase2 candidate recall >=0.85 count: {_count_numeric_ge(summary, 'candidate_recall', 0.85)}",
        f"- no-candidate-path count: {len(manifest.get('no_candidate_path_rule_ids', []))}",
        f"- wave1 activation rule_ids: {', '.join(wave[wave['wave1_decision'] == 'INCLUDE']['rule_id'].astype(str).tolist()) if 'wave1_decision' in wave.columns else ''}",
    ...
    text = "\n".join(lines) + "\n"
    (root / "phase2_final_report.md").write_text(text, encoding="utf-8")
    return text
...
def _under_quota_decision(
    *,
    ...
    if phase2_count >= target and recall_after >= 0.85:
        return "BACKFILLED", f"Candidate-backed examples reached target {target}."
    if not synthetic_generator and phase2_count == 0:
...
def _wave1_decision(item: dict[str, Any], under_decision: str, blockers: list[str]) -> tuple[bool, str]:
    unsafe_blockers = {
        "NEEDS_VALIDATOR",
    ...
    if str(item.get("decision", "")) == "READY_NEXT_DATASET":
        return True, "passes phase2 gates"
    precision = float(_number(item.get("precision"), 0.0))
    recall = float(_number(item.get("recall"), 0.0))
...
def _write_backlog_md(frame: pd.DataFrame, output_path: str | Path) -> None:
    lines = ["# Rule Expansion Backlog V2", "", "Blocked phase2 rules by required next work.", ""]
...
def _write_wave1_md(frame: pd.DataFrame, output_path: str | Path) -> None:
    include = frame[frame["wave1_decision"] == "INCLUDE"]["rule_id"].astype(str).tolist() if not frame.empty else []
    exclude = frame[frame["wave1_decision"] != "INCLUDE"]["rule_id"].astype(str).tolist() if not frame.empty else []
    lines = [
