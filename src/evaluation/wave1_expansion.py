...
def write_verified_wave1_activation_plan(
    *,
    phase2_wave1_path: str | Path,
    phase2_summary_path: str | Path,
    under_quota_path: str | Path | None = None,
    phase2_under_quota_path: str | Path | None = None,
    output_csv_path: str | Path,
    output_md_path: str | Path,
) -> Any:
    wave_rows = _read_csv_dicts(phase2_wave1_path)
    summary_by_rule = {row.get("rule_id", ""): row for row in _read_csv_dicts(phase2_summary_path)}
    quota_path = under_quota_path or phase2_under_quota_path
    ...
    for row in wave_rows:
        ...
        activation = row.get("wave1_decision") or row.get("activation_decision") or "EXCLUDE"
        ...
        if rule_id == "hyphen_whitelist" and activation != "INCLUDE":
            reason = _append_reason(reason, "Phase 2 READY_NEXT_DATASET but eval_count=1, KEEP_UNDER_QUOTA")
    ...
    _write_csv_dicts(output_csv, verified_rows, _wave1_fieldnames())
    ...
    md_lines = [
        ...
        "",
        f"Phase 2 READY_NEXT_DATASET count: {ready_count}",
        f"Verified Wave 1 INCLUDE count: {include_count}",
...
def normalize_phase3_decision(decision: str, blockers: Sequence[str] | str | None = None) -> str:
...
def write_phase3_final_report(*, reports_dir: str | Path, data_dir: str | Path) -> str:
    ...
    for row in rows:
        row["phase3_decision"] = normalize_phase3_decision(
            row.get("phase3_decision") or row.get("decision") or row.get("under_quota_decision") or "",
    ...
    decision_counts = Counter(row.get("phase3_decision", "KEEP_INACTIVE") for row in rows)
    ...
    md_lines = [
        "# Matrix Eval Phase 3 Final Report",
        "",
        f"Verdict: {verdict}",
    ...
    (reports / "phase3_final_report.md").write_text("\n".join(md_lines) + "\n", encoding="utf-8")
    return verdict


def write_working_v1_vs_v3_comparison(
    *,
    working_v1_summary_path: str | Path,
    v3_summary_path: str | Path,
    output_csv_path: str | Path,
    output_md_path: str | Path,
    training_ran: bool,
) -> str:
    baseline = _first_row(working_v1_summary_path)
    candidate = _first_row(v3_summary_path)
    comparison_rows = []
    for metric in COMPARISON_METRICS:
        base_value = _float(baseline.get(metric))
        v3_value = _float(candidate.get(metric))
        comparison_rows.append(
            {
                "metric": metric,
                "working_v1": _format_float(base_value),
                "v3": _format_float(v3_value),
                "delta": _format_float(v3_value - base_value),
            }
        )
    ...
    output_csv = Path(output_csv_path)
    output_csv.parent.mkdir(parents=True, exist_ok=True)
    _write_csv_dicts(output_csv, comparison_rows, ["metric", "working_v1", "v3", "delta"])

    md_lines = [
        "# WORKING_V1 vs v3 Comparison",
        "",
        f"Verdict: {verdict}",
        f"Training ran: {bool(training_ran)}",
        "",
        "| metric | WORKING_V1 | v3 | delta |",
        "|---|---:|---:|---:|",
    ]
    md_lines.extend(
        f"| {row['metric']} | {row['working_v1']} | {row['v3']} | {row['delta']} |" for row in comparison_rows
    )
    output_md = Path(output_md_path)
    output_md.parent.mkdir(parents=True, exist_ok=True)
    output_md.write_text("\n".join(md_lines) + "\n", encoding="utf-8")
...
def _comparison_verdict(baseline: Mapping[str, str], candidate: Mapping[str, str], *, training_ran: bool) -> str:
    ...
    return "PROMOTE_WORKING_V2" if safety_ok and improved else "KEEP_WORKING_V1"


def _wave1_fieldnames() -> list[str]:
    return [
        "rule_id",
        "matrix_group",
        "status",
